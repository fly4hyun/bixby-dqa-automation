...
def parse_opt():
    ...
    ###############################################################################################
    ### 라벨 이미지 폴더 위치
    ### default 작성 시, 기존 라벨이 다 날아갈 수 있으므로 default 항목 제거
    ### ex) --label_path temp_data_1
...
###################################################################################################
### 모든 폴더 처리
def process_all_folders(root_dir, pdf_file, modal_path='weights/best.pt'):
    for folder_name in os.listdir(root_dir):
        folder_path = os.path.join(root_dir, folder_name)
        if not os.path.isdir(folder_path):
            continue
        print(f"[INFO] Processing folder: {folder_name}")
        ### 1. ori_images 폴더 생성 및 원본 이미지 이동
        ori_img_dir = os.path.join(folder_path, "ori_images")
        os.makedirs(ori_img_dir, exist_ok=True)
        for f in os.listdir(folder_path):
            if f.lower().endswith((".jpg", ".jpeg", ".png")):
                src = os.path.join(folder_path, f)
                dst = os.path.join(ori_img_dir, f)
                shutil.move(src, dst)
        ### 2. labels, images 폴더 생성 (ori_images와 동일 위치)
        labels_dir = os.path.join(folder_path, "labels")
        images_dir = os.path.join(folder_path, "images")
        os.makedirs(labels_dir, exist_ok=True)
        os.makedirs(images_dir, exist_ok=True)
        ### 3. PDF 파일: PDF는 "pdf_2" 폴더 내에 있으며, 파일명은 폴더명과 동일하다고 가정
        pdf_file = os.path.join("pdf", f"{folder_name}.pdf")
        if not os.path.exists(pdf_file):
            print(f"[WARNING] PDF file not found: {pdf_file}. Skipping OCR-based processing.")
            pdf_file = ""
        ### 4. ori_images 내 이미지 목록 (모든 이미지)
        img_list = []
        for f in sorted(os.listdir(ori_img_dir)):
            if f.lower().endswith((".jpg", ".jpeg", ".png")):
                img_list.append(os.path.join(ori_img_dir, f))
        if not img_list:
            print(f"[WARNING] No images found in {ori_img_dir}")
            continue
        ### 5. YOLO 모델 로드
        model = YOLO(modal_path)
        ### 6. DetectionYOLO 인스턴스 생성
        detector = DetectionYOLO(model, pdf_file, folder_path)
        ### 7. 객체 탐지 및 후처리 수행 (모든 이미지 처리)
        detector.detect_and_postprocess(img_list, folder_name, images_dir, labels_dir)
...
if __name__ == "__main__":
    opt = parse_opt()
    process_all_folders(opt.label_path, opt.pdf_path, opt.model_path)

...
* PyQt5 기반 **라벨링 툴 개발** 및 문서 레이아웃 탐지 결과 검수용으로 활용
* 75종 PDF 라벨링 (학습 3,649장 / 검증 440장) 및 총 178종 PDF 검수 후 DQA 데이터 생성
* YOLOv12x 기반 문서 레이아웃 탐지 (12 classes 정의 및 섹션박스/이미지박스 추가 설계 근거 포함)
* 레이아웃 탐지 결과를 **문서 읽기 순서(대→중→소제목→내용)**에 맞게 정렬 및 구조화
* PDF로더로 텍스트-레이아웃 영역 매핑 저장
* 동일 제품의 KR–다국어 설명서 매핑하여 **특화용어사전 구축**
* (Note) 구조화 이후 QA 생성은 사내 별도 담당 영역
